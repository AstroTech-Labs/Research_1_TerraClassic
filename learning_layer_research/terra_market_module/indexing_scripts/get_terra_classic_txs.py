...
def get_historical_market_swap_txs(debug=True):

    offset = 285001451  # But was halted a second time at block 7607789
    ...
    try:
        ...
        market_swap_send_txs = pd.DataFrame(
            [], columns=['TxHash', 'BlockHeight', 'Sender', 'ask_denom', 'burner', 'tokens_burnt', 'minter', 'tokens_minted', 'offer_coin', 'trader', 'recipient', 'swap_coin', 'swap_fee'])
    # lockdrop_txs = pd.DataFrame([], columns=['TxHash', 'BlockHeight', 'Sender',
    #                             'ASTRO Deposited', 'xASTRO Deposited', 'ASTRO Withdrawn', 'xASTRO Withdrawn'])
    ...
    try:
        ...
        print(e)
        # lockdrop_txs.to_csv(f'./terra_crash_txs.csv', index=False)

    if debug:
        print(f"DATA COLLECTION TASK SUCCESSFULLY COMPLETED")
